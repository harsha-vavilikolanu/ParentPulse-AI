...
import { Wallet, IndianRupee, Clock, ArrowDownToLine, ReceiptText, ShieldCheck, Award } from 'lucide-react';
...
const Finance = () => {
  ...
  useEffect(() => {
    setTimeout(() => {
      setData(mockStudentData.finance);
    }, 400);
  }, []);
  ...
  return (
    ...
    >
      <div className="mb-8">
        ...
        <p className="text-slate-500 mt-1">Manage fee payments, view transaction history, and check scholarships.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <GlassCard className="col-span-1 md:col-span-2 lg:col-span-2 bg-gradient-to-br from-primary to-primary/80 text-white border-none flex flex-col justify-between">
          <div>
            ...
            <div className="flex items-center gap-2 mt-2">
              ...
              <h2 className="text-5xl font-bold">{data.pending.toLocaleString('en-IN')}</h2>
            </div>
          </div>
        ...
        <GlassCard className="flex flex-col justify-between">
          ...
          <div className="mt-4 pt-4 border-t border-slate-100 text-sm">
            <span className="text-emerald-600 font-medium">{Math.round((data.paid / data.totalFee) * 100)}%</span> of total fee cleared
          </div>
        </GlassCard>
        ...
        <GlassCard className="flex flex-col justify-between">
          <div>
            ...
            <div className="flex items-center gap-1">
              <IndianRupee className="w-5 h-5 text-slate-800" />
              <span className="text-2xl font-bold text-slate-800">{data.totalFee.toLocaleString('en-IN')}</span>
            </div>
          </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200">
                    <th className="py-3 px-4 font-semibold text-sm text-slate-600">ID / Date</th>
                    <th className="py-3 px-4 font-semibold text-sm text-slate-600">Description</th>
                  ...
                  {data.transactions.map((txn, idx) => (
                    <tr key={idx} className="hover:bg-slate-50/50 transition-colors">
                      <td className="py-4 px-4">
                        <div className="font-semibold text-slate-800 text-sm">{txn.id}</div>
                        <div className="text-xs text-slate-500 flex items-center gap-1 mt-1"><Clock className="w-3 h-3"/> {txn.date}</div>
                      </td>
                      <td className="py-4 px-4 text-sm text-slate-700">{txn.purpose}</td>
                      <td className="py-4 px-4">
          ...
          <GlassCard>
            ...
            {data.scholarships.length > 0 ? (
              <div className="space-y-4">
                {data.scholarships.map((schol, idx) => (
                  <div key={idx} className="bg-accent/10 border border-accent/20 rounded-xl p-5 relative overflow-hidden">
                    <div className="absolute -right-4 -top-4 opacity-10">
                      <Award className="w-24 h-24 text-accent" />
                    </div>
                    <h4 className="font-bold text-slate-800 text-lg mb-1 relative z-10">{schol.name}</h4>
                    <p className="text-sm text-slate-500 relative z-10 mb-4">Academic Year: {schol.year}</p>
                    <div className="flex items-center text-accent-dark font-black text-2xl relative z-10">
                      <IndianRupee className="w-6 h-6 mr-1" />
                      {schol.amount.toLocaleString('en-IN')}
                    </div>
                    <div className="text-xs font-semibold uppercase tracking-wider text-emerald-600 mt-2">Applied to Fee Account</div>
                  </div>
                ))}
              </div>
            ) : (
              <div className="text-center py-8">
                <p className="text-slate-500">No active scholarships.</p>
              </div>
            )}
            
            <button className="btn-outline w-full mt-6 py-2.5 text-sm flex justify-center items-center gap-2">
              <ArrowDownToLine className="w-4 h-4"/> View Fee Receipt Structure
            </button>
          </GlassCard>
        </div>
      </div>
